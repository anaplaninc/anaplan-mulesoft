...
//	}
	...
	private static AnaplanResponse executeAction(Model model, String actionId, 
		...
		if(status.getTaskState() == TaskStatus.State.COMPLETE && status.getResult().isSuccessful()){
			...
			return AnaplanResponse.executeActionSuccess(status.getTaskState().name(), logContext);
		}else{
...
//	}
	...
	public static AnaplanResponse runImportCsv(AnaplanConnection connection,
			String importId, InputStream is, String logContext, UserLog userLog, String columnSeparator)
		...
		if (serverFile != null) {
			...
			List<String[]> rows = parseImportData(is,row_count,columnSeparator);
	
			/*final CellWriter dataWriter = serverFile.getUploadCellWriter(serverFile
					.getSeparator());*/
			...
			for (String[] row : rows) {
				...
				LogUtil.trace(logContext, rowsProcessed + "-" + debug_output(row));
				++rowsProcessed;
			}
			dataWriter.close();
		}
	...
	public static List<String[]> parseImportData(InputStream is, int row_count, String columnSeparator)
			throws IOException {
		String line;
		List<String[]> rows = new ArrayList<String[]>();
		BufferedReader reader = new BufferedReader(new InputStreamReader(is));
